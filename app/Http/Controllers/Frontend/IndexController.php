...
class IndexController extends Controller
{
    ...
    public function CategoryProduct(Request $request,$id,$slug){
      ...
      return view('frontend.product.category_view',compact('products','categories','getcat','newProduct'));

  
       }// End Method
       public function SubCategoryProduct(Request $request,$id,$slug){
        $products = Product::where('status',1)->where('subcategory_id',$id)->orderBy('id','DESC')->get();
        $categories = Category::orderBy('category_name','ASC')->get();
  
        $getsubcat = SubCategory::where('id',$id)->first();
  
        $newProduct = Product::orderBy('id','DESC')->limit(3)->get();
  
        return view('frontend.product.subcategory_view',compact('products','categories','getsubcat','newProduct'));
