...
<head>
    <meta charset="utf-8" />
    <title>Valerie Online Shop</title>
...
//   }
  ...
  $.ajax({
    ...
    error: function(xhr, textStatus, errorThrown) {
      ...
      Toast.fire({
        icon: 'error',
        title: 'An error occurred while adding the item to the cart. Please try again.'
      });
    }
  });
}
    ...
    function addToCartDetails(){
     var product_name = $('#details_productname').text();  
     var id = $('#details_product_id').val();
     var color = $('#details_color option:selected').text();
     var size = $('#details_size option:selected').text();
     var quantity = $('#details_qty').val(); 
     $.ajax({
        type: "POST",
        dataType : 'json',
        data:{
        ...
        url: "/details_cart/data/store/"+id,
        success:function(data){
            miniCart();
...
?`<h3 class="text-brand">MMK${value.product.selling_price}</h3>`
:`<h3 class="text-brand">MMK${value.product.discount_price}</h3>`
}
...
<script type="text/javascript">
    ...
    <td class="row_img"><img src="/${value.product.product_thumbnail} " style="width:300px; height:300px;"  alt="compare-img" /></td>
                      ...
                      ${value.product.discount_price == null
                        ? `<h4 class="price text-brand">MMK${value.product.selling_price}</h4>`
                        :`<h4 class="price text-brand">MMK${value.product.discount_price}</h4>`
                        } 
                                    </td>
...
rows += `  <tr class="pt-30">
            ...
            <td class="price" data-title="Price">
                <h4 class="text-body">MMK${value.price} </h4>
            </td>
              <td class="price" data-title="Price">
      ...
      <a  type="submit" class="qty-up" id="${value.rowId}" onclick="cartIncrement(this.id)"><i class="fi-rs-angle-small-up"></i></a>
            ...
            <td class="price" data-title="Price">
                <h4 class="text-brand">MMK${value.subtotal} </h4>
            </td>
            <td class="action text-center" data-title="Remove">
      ...
      $('#showtotal').html(
        `<tr>
          ...
          <td class="cart_total_amount">
            <h4 class="text-brand text-end">MMK${data.subtotal}</h4>
          </td>
        </tr>
        <tr>
          <td class="cart_total_label">
          ...
          <td class="cart_total_amount">
            <h4 class="text-brand text-end">MMK${data.total}</h4>
          </td>
        </tr>`
      );
    }
...
//       var text = (count == 1 ? 'product' : 'products');
//       $('#cart-count-number').text(count);
//       $('#cart-count-text').text(text);
//     }
//   });
// }
...
function updateCartCount() {
  $.ajax({
    ...
    success: function(data) {
      var count = data.count;
      var text = (count == 1 ? 'product' : 'products');
      var isOrAre = (count == 1 ? 'is' : 'are');
      var message = '';

      if (count == 0) {
        message = 'There is no product found';
      } else {
        message = 'There ' + isOrAre + ' <span id="cart-count-number">' + count + '</span> <span id="cart-count-text">' + text + '</span> in your cart';
      }
      $('#cart-count').html(message);
    }
  });
}
